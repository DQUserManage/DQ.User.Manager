...
protected:
	...
	//lqt 导出类
	class BCGCBPRODLLEXPORT CHeaderLayout
	{
	public:
	...
	public:
		...
		//lqt 导出类
		struct BCGCBPRODLLEXPORT HeaderLayoutData
		{
...
public:
	...
	//lqt 20180612 改成虚函数，目的是测试排序时允许合并单元格
	virtual void MergeRange (const CBCGPGridRange& range, BOOL bRedraw = TRUE);
...
protected:
	...
	//lqt 2018.06.12  改为虚函数，目的是当排序时可以合并单元格
	virtual void UpdateMergedItems ();
	void RedrawMergedItem (CBCGPGridItem* pItem);
	void RedrawMergedItems (CDC* pDC);
	//lqt 2018.06.01 改为虚函数，目的是适应自定义行高
	virtual CRect GetMergedRect (const CBCGPGridRange* pRange, const CBCGPGridItemID& id);//TODO replace the 2nd parameter
